...
{
    /// <summary>
    /// The canonical command. Long enough that no other plugin is likely to have taken it.
    /// </summary>
    private const string CommandName = "/fatecompass";

    /// <summary>
    /// The one people will actually type.
    /// </summary>
    /// <remarks>
    /// Registered as well as, not instead of, the long form. A short command is worth having and
    /// is exactly the kind another plugin might already own, so this one is allowed to fail: if
    /// it does, the long form still works and the plugin is still usable.
    /// <para>
    /// Not <c>/fc</c>, which the game itself uses for free company chat.
    /// </para>
    /// </remarks>
    private const string ShortCommandName = "/fate";

    /// <summary>Whether <see cref="ShortCommandName"/> was actually claimed, so teardown matches setup.</summary>
    private readonly bool shortCommandRegistered;

    private readonly WindowSystem windowSystem = new("FateCompass");
...
#pragma warning restore CS0162
        ...
        {
            HelpMessage = $"Open the Fate Compass window. Use {ShortCommandName} help for the full list.",
        });

        shortCommandRegistered = DalamudServices.CommandManager.AddHandler(
            ShortCommandName,
            new CommandInfo(OnCommand)
            {
                HelpMessage = $"Short form of {CommandName}.",
            });

        if (!shortCommandRegistered)
        {
            DalamudServices.Log.Information(
                "{Short} is taken by another plugin; {Long} is still available.",
                ShortCommandName,
                CommandName);
        }
    ...
    public void Dispose()
    {
        ...
        if (shortCommandRegistered)
        {
            DalamudServices.CommandManager.RemoveHandler(ShortCommandName);
        }
    ...
    private void ReportSpeed()
    {
        ...
        if (average is null)
        {
            DalamudServices.ChatGui.Print("[Fate Compass] Not enough movement measured yet. Travel a while first.");
            return;
        }
        ...
        DalamudServices.ChatGui.Print(
            $"[Fate Compass] Measured in {content}: average {average.Value:F1} y/s, " +
        ...
        DalamudServices.ChatGui.Print(
            $"[Fate Compass] Stored {speed.PeakYalmsPerSecond:F1} y/s for this zone " +
    ...
    private void RunDiagnostics(string what)
    {
        switch (what)
        {
            ...
            case "progress":
                ...
                DalamudServices.ChatGui.Print("[Fate Compass] FATE progress dumped to /xllog.");
                break;

            case "mount":
            case "ridingmap":
                Diagnostics.DumpMountSpeed();
                DalamudServices.ChatGui.Print("[Fate Compass] Mount speed dumped to /xllog.");
                break;

            case "events":
            case "dynamic":
                Diagnostics.DumpDynamicEvents();
                DalamudServices.ChatGui.Print("[Fate Compass] Dynamic events dumped to /xllog.");
                break;

            case "map":
            case "marker":
                Diagnostics.ProbeMapMarkers();
                DalamudServices.ChatGui.Print("[Fate Compass] Map marker probe written to /xllog.");
                break;
            ...
            case "speedreset":
                ...
                DalamudServices.ChatGui.Print("[Fate Compass] Speed measurement reset. Fly around, then /fate debug speed.");
                break;

            default:
                DalamudServices.ChatGui.Print(
                    "[Fate Compass] /fate debug progress  (open the FATE progress window first)");
                DalamudServices.ChatGui.Print(
                    "[Fate Compass] /fate debug map  (then open the map and see what appeared)");
                DalamudServices.ChatGui.Print(
                    "[Fate Compass] /fate debug speed  (measured travel speed, /fate debug speedreset to start over)");
                break;
        }
    }
